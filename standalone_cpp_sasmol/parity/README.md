...
| PDB I/O | `PdbReader` / `PdbWriter` | implemented | Implemented for tolerant fixed-column reads, END and MODEL trajectories, selected/all-frame writes, CONECT output, optional-field fill, and larger fixture validation through Python `zazmol`. Full element-resolution parity remains deferred. |
