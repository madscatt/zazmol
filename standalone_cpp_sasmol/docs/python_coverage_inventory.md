...
| `subset.Mask.get_subset_mask` open Python eval behavior | `test_subset/*get_subset_mask*`, SASSIE basis survey | `sasmol/selection.hpp` | `intentional difference` | high | C++ supports the surveyed SASSIE/VMD-like basis language through a safe translator, but still rejects arbitrary Python `eval`. |
