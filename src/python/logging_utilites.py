...
class run_utils():
    ...
    def write_json_file(self):
        ...
        with open('.last_sas_' + self.__application__ + '.json', 'w') as outfile:
            json.dump([self.v], outfile)
            self.logger.info('writing parameters to .last_sas_' +
                             self.__application__ + '.json file')
    ...
    def general_setup(self, other_self):
        ...
        input_vars = [attr for attr in dir(mvars) if not callable(
            getattr(mvars, attr)) and not attr.startswith("__")]
    ...
    def preamble(self):
        ...
        ##self.logger.info('sasmol version(sasmol) : '+sasmol.__version__)
        #self.logger.info('sasmol version : '+pkg_resources.get_distribution("sasmol").version)
    ...
    def setup_logging(self, other_self):
        ...
        self.logfile = self.__application__ + '_' + timestr + '.sasmol_log'
