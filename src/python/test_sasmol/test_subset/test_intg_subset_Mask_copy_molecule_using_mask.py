...
class Test_subset_Mask_copy_molecule_using_mask(unittest.TestCase):
    ...
    def generate_expected_pdb(self, o, mask, frame):
        self.o_expected.setAtom(
            [y for (x, y) in zip(mask, self.o.atom()) if x == 1])
        self.o_expected.setIndex(
            [y for (x, y) in zip(mask, self.o.index()) if x == 1])
        self.o_expected.setName(
            [y for (x, y) in zip(mask, self.o.name()) if x == 1])
        self.o_expected.setLoc(
            [y for (x, y) in zip(mask, self.o.loc()) if x == 1])
        self.o_expected.setResname(
            [y for (x, y) in zip(mask, self.o.resname()) if x == 1])
        self.o_expected.setChain(
            [y for (x, y) in zip(mask, self.o.chain()) if x == 1])
        self.o_expected.setResid(
            [y for (x, y) in zip(mask, self.o.resid()) if x == 1])
        self.o_expected.setRescode(
            [y for (x, y) in zip(mask, self.o.rescode()) if x == 1])
        self.o_expected.setOccupancy(
            [y for (x, y) in zip(mask, self.o.occupancy()) if x == 1])
        self.o_expected.setBeta(
            [y for (x, y) in zip(mask, self.o.beta()) if x == 1])
        self.o_expected.setSegname(
            [y for (x, y) in zip(mask, self.o.segname()) if x == 1])
        self.o_expected.setElement(
            [y for (x, y) in zip(mask, self.o.element()) if x == 1])
        self.o_expected.setCharge(
